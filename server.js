...
const serializeUsers = () => [...users.values()].map(u => ({
  id: u.id, name: u.name, status: u.status, avatar: u.avatar
}));

const serializeRooms = () => [...rooms.values()].map(r => ({
  id: r.id, name: r.name, members: r.members, pinned: r.pinned
}));
