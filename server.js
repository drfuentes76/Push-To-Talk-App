// server.js — GitHub/Render ready (full features)
...
const io = new Server(server, { cors: { origin: process.env.ORIGIN || '*'}, path: '/socket.io', maxHttpBufferSize: 2e8 });

app.get(process.env.HEALTHCHECK_PATH || '/healthz', (_req,res)=>res.status(200).send('ok'));
